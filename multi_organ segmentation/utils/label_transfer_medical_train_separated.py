...
ORGAN_DATASET_DIR = '/public1/cuikangjun/Data_Seg_Separated/'
NUM_WORKER = 8
...
LABEL_MAP = {
    'LiTS':{1:1, 2:1},
    'spleen':{1:1, 2:1},
    'pancreas':{1:1, 2:1, 3:1},
    'KiTS':{4:1, 5:2},                            # 4:Right Kidney, 5:Left Kidney
    'AMOS':{4:1, 6:1},                            # train
    'BTCV':{6:1, 2:4, 3:5, 11:3, 1:2, 4:6},
    'AMOS_test':{6:1, 2:4, 3:5, 10:3, 1:2, 4:6},  # test
}
...
def combine_labels(real_label, dataset_name):
    """
    Convert class index tensor to one hot encoding tensor.
    Args:
        real_label: A tensor of shape [bs, 1, *]
    ...
    # 4.合并真实标签和伪标签
    combined_labels = np.where(mapped_labels > 0, mapped_labels, mapped_labels)
...
for index, batch in enumerate(train_loader):
    ...
    y = combine_labels(y, DATASET_NAME)
    # print(f"y:{y.shape}")
