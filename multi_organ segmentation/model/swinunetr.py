...
class SwinUNETR(nn.Module):
    ...
    #     # 获取第一类（背景类）的权重和偏置
    #     cls = self.out.conv.conv  # 这是UnetOutBlock中的最后一个卷积层
    #     imprinting_w = cls.weight[0]  # 获取第一层滤波器（背景类）
    #     bkg_bias = cls.bias[0]  # 获取背景类的偏置
    #
    #     # 计算新的偏置
    #     bias_diff = torch.log(torch.FloatTensor([self.new_classes + 1])).to(device)
    #     new_bias = (bkg_bias - bias_diff)
    #
    #     # 将新计算的权重和偏置应用到最后一个类别（cls[0]对应背景类）
    #     cls.weight.data.copy_(imprinting_w.clone())
    #     cls.bias.data.copy_(new_bias)
    #
    #     # 调整背景类的偏置
    #     self.out.conv.conv.bias[0].data.copy_(new_bias.squeeze(0))

    def forward(self, x_in, return_feature=False):

        cls_weight = self.out.conv.conv.weight
        ...
        if self.training or return_feature:
            return [enc0, enc1, enc2, enc3, dec4, dec3, dec2, dec1, dec0, out, logits], cls_weight
        else:
            return logits
