...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        if self.encoding == 'rand_embedding':
            ...
            #self.register_buffer('organ_embedding', torch.randn(6, 512))
        ...
        for iclass in range(self.class_num - 1):
            self.controllers.append(
                nn.Sequential(
    ...
    def forward(self, x_in, return_feature=False):
        ...
        for icls in range(self.class_num):
            if icls == 0:
                # print("Background don't use organ_embedding")
                x_cond = x_feat
            else:
                x_cond = torch.cat([x_feat, task_encoding[icls-1].unsqueeze(0).expand(b, -1, -1, -1, -1)], dim=1)
        ...
        if self.training or return_feature:
            return [enc0, enc1, enc2, enc3, dec4, dec3, dec2, dec1, dec0, out, logits_array], class_params
        else:
            return logits_array
