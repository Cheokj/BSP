...
from utils.utils import dice_score, threshold_organ, visualize_label, merge_label, get_key, cal_asd, cal_hd
...
def validation(model, ValLoader, val_transforms, args):
    ...
    for key in TEMPLATE.keys():
        ...
        hd95_list[key] = np.zeros((2, NUM_CLASS))
        asd_list[key] = np.zeros((2, NUM_CLASS))
    percase_result_str = ''
    ...
    for index, batch in enumerate(tqdm(ValLoader)):
        ...
        with torch.no_grad():
            ...
            if args.out_nonlinear == 'sigmoid':
                ...
                pred_hard = (pred_sigmoid > 0.5).int()
        ...
        for b in range(B):
            ...
            for organ in args.organ_list:
                if organ-1 < label.shape[1] and torch.sum(label[b,organ-1,:,:,:].cuda()) != 0:
                    dice_organ, recall, precision = dice_score(pred_hard_post[b,organ-1,:,:,:].cuda(), label[b,organ-1,:,:,:].cuda())
                    hd95, asd = 0, 0
                    if torch.sum(pred_hard_post[b,organ-1,:,:,:].cuda()) != 0:
                        hd95 = cal_hd(pred_hard_post[b,organ-1,:,:,:].cpu(), label[b,organ-1,:,:,:].cpu())
                        asd = cal_asd(pred_hard_post[b,organ-1,:,:,:].cpu(), label[b,organ-1,:,:,:].cpu())
                        hd95_list[template_key][0][organ-1] += hd95
                        hd95_list[template_key][1][organ-1] += 1
                        asd_list[template_key][0][organ-1] += asd
                        asd_list[template_key][1][organ-1] += 1

                    dice_list[template_key][0][organ-1] += dice_organ.item()
                    dice_list[template_key][1][organ-1] += 1
                    content += f"{ORGAN_NAME[organ-1]}: {dice_organ.item():<6.4f} {hd95:<6.2f} {asd:<4.4f}, "
                    print('%s: dice %.4f, recall %.4f, precision %.4f, hd95 %.4f, asd %.4f.'%(ORGAN_NAME[organ-1], dice_organ.item(), recall.item(), precision.item(), hd95, asd))
            print(content)
    ...
    ave_organ_asd = np.zeros((2, NUM_CLASS))

    file_base_name = name[0].split('/')[0]
    with open(os.path.join(save_dir, f'result_{file_base_name}.txt'), 'w') as f:
        for key in TEMPLATE.keys():
            # organ_list = TEMPLATE[key]
            content = 'Task%s| '%(key)
            for organ in args.organ_list:
                dice = dice_list[key][0][organ-1] / (dice_list[key][1][organ-1] + 1e-6)  ## 具体任务下器官的平均值
                content += '%s: %.4f, '%(ORGAN_NAME[organ-1], dice)
                ave_organ_dice[0][organ-1] += dice_list[key][0][organ-1]    ## 所有任务下器官的平均值
                ...
                ave_organ_asd[0][organ-1] += asd_list[key][0][organ-1]
                ave_organ_asd[1][organ-1] += asd_list[key][1][organ-1]
            print(content)
            f.write(content)
            f.write('\n')
        content = 'Average:  ' + '\n'
        content += f"{'organ':<12} {'dice':<8} {'hd95':<8} {'asd':<8}\n"
        for i in args.organ_list:
            content += f"{ORGAN_NAME[i-1]:<12} {ave_organ_dice[0][i-1] / ave_organ_dice[1][i-1]:<8.4f} {ave_organ_hd95[0][i-1]/ave_organ_hd95[1][i-1]:<8.2f} {ave_organ_asd[0][i-1]/ave_organ_asd[1][i-1]:<8.4f}\n"
        print(content)
        f.write(content)
        f.write('\n')
